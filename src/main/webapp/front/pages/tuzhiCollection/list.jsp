            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">图纸收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="tuzhiName" id="tuzhiName" placeholder="图纸标题" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="tuzhiTypes" id="tuzhiTypes" lay-filter="tuzhiTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in tuzhiTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../tuzhi/detail.jsp?id='+item.tuzhiId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.tuzhiPhoto?item.tuzhiPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../tuzhi/detail.jsp?id='+item.tuzhiId)"><span>{{item.tuzhiName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.tuzhiNewMoney"><span style="color: red;">{{item.tuzhiNewMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteTuzhiCollection(item.id)">删除 {{item.tuzhiName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 tuzhiCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            tuzhiCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteTuzhiCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('tuzhiCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#tuzhiName').val() != null && jquery('#tuzhiName').val() != "" && jquery('#tuzhiName').val() != "null" ) {
                param['tuzhiName'] = jquery('#tuzhiName').val();
            }
                                                             
            var tuzhiTypes = $("#tuzhiTypes").find("option:selected").val();
            if (tuzhiTypes != null && tuzhiTypes != "" && tuzhiTypes != "null") {
                            param['tuzhiTypes'] = tuzhiTypes;
                        }
                                                                                
            // 当前表
                                                    // 获取列表数据
            http.request('tuzhiCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function tuzhiTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=tuzhi_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.tuzhiTypesList = res.data.list;
               }
           });
       }
