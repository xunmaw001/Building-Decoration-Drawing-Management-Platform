            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑设计师</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">设计师信息</h3>
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input id="shejishiIdNumber" name="shejishiIdNumber" class="form-control"
                                               onchange="shejishiIdNumberChickValue(this)"  placeholder="身份证号">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>手机号</label>
                                        <input id="shejishiPhone" name="shejishiPhone" class="form-control"
                                               onchange="shejishiPhoneChickValue(this)"  placeholder="手机号">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>照片</label>
                                        <img id="shejishiPhotoImg" src="" width="100" height="100">
                                        <input name="file" type="file" id="shejishiPhotoupload"
                                               class="form-control-file">
                                        <input name="shejishiPhoto" id="shejishiPhoto-input" type="hidden">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        <!-- 当前表的文件上传 -->
        $('#shejishiPhotoupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                var photoUrl= baseUrl + 'file/download?fileName=' + data.result.file;
                document.getElementById('shejishiPhotoImg').setAttribute('src',photoUrl);
                document.getElementById('shejishiPhoto-input').setAttribute('value',photoUrl);
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("shejishi/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addshejishi');
    ...
    //数字检查
        <!-- 当前表的数字检查 -->
        function shejishiIdNumberChickValue(e){
        ...
        function shejishiPhoneChickValue(e){
            var this_val = e.value || 0;
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                username: "required",
                password: "required",
                shejishiName: "required",
                sexTypes: "required",
                shejishiIdNumber: "required",
                shejishiPhone: "required",
                shejishiPhoto: "required",
            },
            messages: {
                username: "账户不能为空",
                password: "密码不能为空",
                shejishiName: "设计师姓名不能为空",
                sexTypes: "性别不能为空",
                shejishiIdNumber: "身份证号不能为空",
                shejishiPhone: "手机号不能为空",
                shejishiPhoto: "照片不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addshejishi = window.sessionStorage.getItem("addshejishi");
        if (addshejishi != null && addshejishi != "" && addshejishi != "null") {
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("shejishi/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
