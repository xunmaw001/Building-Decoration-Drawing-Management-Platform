    ...
    */
    ...
    public R info(@PathVariable("id") Long id){
        ...
        if(dingdan !=null){
            ...
            //级联表
            TuzhiEntity tuzhi = tuzhiService.selectById(dingdan.getTuzhiId());
            if(tuzhi != null){
                BeanUtils.copyProperties( tuzhi , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
                view.setTuzhiId(tuzhi.getId());
            }
            //级联表
            TuzhiOrderEntity tuzhiOrder = tuzhiOrderService.selectById(dingdan.getTuzhiOrderId());
            if(tuzhiOrder != null){
                BeanUtils.copyProperties( tuzhiOrder , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
                view.setTuzhiOrderId(tuzhiOrder.getId());
            }
            //级联表
    ...
    */
    ...
    public R save(@RequestBody DingdanEntity dingdan, HttpServletRequest request){
        ...
        Wrapper<DingdanEntity> queryWrapper = new EntityWrapper<DingdanEntity>()
            .eq("tuzhi_order_id", dingdan.getTuzhiOrderId())
            .eq("yonghu_id", dingdan.getYonghuId())
            .eq("tuzhi_id", dingdan.getTuzhiId())
            ;
